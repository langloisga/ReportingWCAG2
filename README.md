...
The purpose of this tool is to generate testing results into an Excel spreadsheet.  The 3 main tools required with the Reporting Tool are: WPSS, CSE HTML Validator Pro and W3C code validator.  All tests can be performed using each single tool separately. 
...
You can verify if the Installation is completed by locate the QA-WS-Tool directory found at the ROOT of C drive. Ensure that the WPSS Tool and the CSE HTML Validator Pro 14 are correctly installed and that the following 5 files exist in your C:\QA-WS-Tool directory: 
